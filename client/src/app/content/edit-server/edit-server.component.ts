...
export class EditServerComponent implements OnInit {
  ...
  editServerFormGroup = new FormGroup({
    serverNameControl: new FormControl({ value: '', disabled: true }, [
      Validators.required,
      Validators.maxLength(50),
    ]),
    serverIpControl: new FormControl('', [Validators.required]),
    serverPortControl: new FormControl(8080, [
      Validators.required,
      Validators.min(0),
      Validators.max(99999),
    ]),
    serverPasswordControl: new FormControl({ value: '', disabled: true }),
    serverSslEnabledControl: new FormControl({
  ...
  ngOnInit(): void {
    //Load server info
    const savedServer: ServerDto | undefined = this.storageService.getServer(
      this.serverNameBeingEdited,
    );
    if (savedServer) {
      //Parse address and port
      const addressWithPort = savedServer.serverURI
        .replace('wss://', '')
        .replace('ws://', '');
      const address = addressWithPort.slice(
        0,
        addressWithPort.lastIndexOf(':'),
      );
      const port = addressWithPort.slice(addressWithPort.lastIndexOf(':') + 1);

      //Set values
      this.editServerFormGroup
        .get('serverNameControl')
        ?.setValue(savedServer.serverName);
  ...
  onUpdatePasswordCheckboxChange(): void {
    ...
    } else this.editServerFormGroup.get('serverPasswordControl')?.enable();
  }
  ...
  saveServer(modal?: any): void {
    ...
    const serverName: string = this.editServerFormGroup
      .get('serverNameControl')
      ?.value.replace(/</g, '&lt;')
      .replace(/>/g, '&gt;')
      .replace(/'/g, '')
    ...
    const serverIp: string =
      this.editServerFormGroup.get('serverIpControl')?.value;
    const serverPort: string =
      this.editServerFormGroup.get('serverPortControl')?.value;
    const serverPassword: string =
      this.editServerFormGroup.get('serverPasswordControl')?.value || null;
    const serverSsl: boolean = this.editServerFormGroup.get(
    ...
    if (this.asModal) modal?.close('Save server');
  }
}
