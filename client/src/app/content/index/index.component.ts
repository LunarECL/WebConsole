...
export class IndexComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.currentlyConnectedServers =
      this.webConsoleService.getCurrentConnectedServers();
  ...
  openModal(modalContent: TemplateRef<any>) {
    this.modalService.open(modalContent, { size: 'lg' }).result.then(
      (result) => {
        //Fullfilled
        this.refreshServerList();
      },
  ...
  openOffcanvas(offcanvasContent: TemplateRef<any>) {
    this.offcanvasService
      .open(offcanvasContent, {
        ariaLabelledBy: 'offcanvas-server-details',
        position: 'end',
      })
      .result.then(
        (result) => {
  ...
  moveServerUp(serverName: string): void {
    const currentIndex: number = this.servers.findIndex(
