...
export class AddServerComponent implements OnInit {
  ...
  saveServer(modal?: any): void {
    ...
    const serverName: string = this.addServerFormGroup
    ...
    const serverIp: string =
      this.addServerFormGroup.get('serverIpControl')?.value;
    const serverPort: string =
      this.addServerFormGroup.get('serverPortControl')?.value;
    const serverPassword: string =
      this.addServerFormGroup.get('serverPasswordControl')?.value || undefined;
    const serverSsl: boolean = this.addServerFormGroup.get(
    ...
    this.serverAlreadyExists =
      this.storageService
        .getAllServers()
        .find((e) => e.serverName == serverName) != undefined;
