...
export class StorageService {
  ...
  public getAllServers(): ServerDto[] {
    var storageObj = JSON.parse(
      window.localStorage['WebConsole'],
    ) as StoredDataDto;
  ...
  public saveServer(
    ...
    if (server) {
      ...
      if (serverPassword) (server as ServerDto).serverPassword = serverPassword;
  ...
  moveServerToIndex(serverName: string, newIndex: number): void {
    ...
    const serverIndex = listOfServers.findIndex(
      (e) => e.serverName == serverName,
    );
    if (serverIndex === -1) return;

    //Move server
    const serverToMove: ServerDto = listOfServers.find(
      (e) => e.serverName == serverName,
    ) as ServerDto;
